...
<style>
    ...
    [data-testid=stSidebar] h1, [data-testid=stSidebar] .stText, [data-testid=stSidebar] .stMarkdown {
        color: #ffffff !important;
    }

    /* Text and markdown elements */
    .markdown-text-container, .stMarkdown, .stText, .stHeader, h1, h2, h3, h4, h5, h6, p, span, li {
    ...
    input, .stTextInput>div>div>input, .stChatInput input {
        background-color: #2a2a3c !important;
        color: #f5f5f5 !important;
        border: 1px solid #5a5afc !important;
    }
...
# Imports images
st.image('./images/Agentic_analysts icon small.png', width=70)
st.title("Agentic-Analyst")
    

# asthetic features for streamlit app
st.logo('./images/Agentic_analysts icon small.png')
st.sidebar.title(":white[Agentic-Analyst] ")
...
def intialize_agent():
    ...
    return agentic_analyst(agents=agent_names,retrievers=retrievers)

#intializes the independent components
@st.cache_resource
def initial_agent_ind():
    return agentic_analyst_ind(agents=agent_names,retrievers=retrievers)
...
def run_chat():
    ...
    placeholder_text = "Welcome to Agentic-Analyst, How can I help you? You can use @agent_name to call a specific agent or let the planner route the query!"
    ...
    # this is the planner based routing
        if specified_agents==[]:
            ...
            with st.chat_message("Agentic-Analyst Bot",avatar="🚀"):
